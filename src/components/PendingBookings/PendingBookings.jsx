...
const UnapprovedBookings = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://192.168.91.63:5500/api/appointments/getallappointments');
      if (!response.ok) {
        throw new Error(`Failed to fetch data: ${response.status} ${response.statusText}`);
      }
      const data = await response.json();
      setRows(data);
      setFilteredRows(data); // Ensure filteredRows initially contains all data
      setError(null);
    } catch (error) {
  ...
  const handleApprove = async (index) => {
    ...
    try {
      ...
      const response = await fetch(`http://192.168.91.63:5500/api/appointments/approvebookings/${rowToApprove.appointmentId}`, {
        method: 'PUT',
        headers: {
      ...
      // Remove the approved booking from the unapproved bookings list
      const newFilteredRows = filteredRows.filter((_, i) => i !== index);
      setFilteredRows(newFilteredRows);
      setRows(rows.filter(row => row.appointmentId !== rowToApprove.appointmentId));
  ...
  return (
    <div style={{ maxWidth: '100vw', overflowX: 'hidden', backgroundColor: '#fff' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
        <h3 style={{ paddingLeft: '5vw', color: '#000' }}>All Unapproved Bookings</h3>
      </div>
      {error ? (
